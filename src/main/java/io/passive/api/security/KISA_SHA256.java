package io.passive.api.security;
...
public class KISA_SHA256 {
	...
	private static final void FF(int[] abcdefgh, int a, int b, int c, int d, int e, int f, int g, int h, int[] X,
		...
		T1 = Common.intToUnsigned(abcdefgh[h]) + Common.intToUnsigned(Sigma1(abcdefgh[e]))
				+ Common.intToUnsigned(Ch(abcdefgh[e], abcdefgh[f], abcdefgh[g])) + Common.intToUnsigned(SHA256_K[j])
	...
	/**
	 * SHA256_Init, SHA256_Process, SHA256_Close 함수를 내부적으로 모두 호출하여 간편한 인터페이스를 제공하기
	 * 위한 함수 매개변수 : pszMessage :입력 메시지의 포인터변수 uPlainTextLen: 입력 메시지의 바이트 길이
